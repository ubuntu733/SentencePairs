...
def evaluation(sess, valid_graph, devDataStream, outpath=None, label_vocab=None):
    ...
    for batch_index in xrange(devDataStream.get_num_batch()):  # for each batch
        ...
        if outpath is not None:
            ...
            for i in xrange(cur_batch.batch_size):
                ...
                write_file.write(sentence1 + '\t')
                write_file.write(sentence2 + '\t')
                write_file.write(label + '\t')
                write_file.write(label_vocab.getWord(predictions[i]) + '\n')
                label_list.extend([int(label)])
                predict_list.extend([int(label_vocab.getWord(predictions[i]))])
    ...
    return accuracy, label_list, predict_list

def train(sess, saver, train_graph, valid_graph, trainDataStream, devDataStream, options, best_path,label_vocab=None):
    best_f1score = -1
    for epoch in range(options.max_epochs):
        ...
        # evaluation
        start_time = time.time()
        acc, labels, predicts = evaluation(sess, valid_graph, devDataStream, outpath='result.json', label_vocab=label_vocab)
        duration = time.time() - start_time
        ...
        print(
            "验证集 classification_report: \n {}".format(
                metrics.classification_report(
                    np.array(labels), np.array(predicts)
                )
            )
        )
        print(
...
def predict(sess, valid_graph, devDataStream, outpath=None, label_vocab=None):
    ...
    for batch_index in xrange(devDataStream.get_num_batch()):  # for each batch
        cur_batch = devDataStream.get_batch(batch_index)
        feed_dict = valid_graph.create_test_feed_dict(cur_batch, is_training=True)
...
def main(FLAGS):
    ...
    # build vocabs
    word_vocab = Vocab(word_vec_path, fileformat='txt')
...
#             if not var.name.startswith("Model"): continue
        ...
        # training
        train(sess, saver, train_graph, valid_graph, trainDataStream, devDataStream, FLAGS, best_path, label_vocab=label_vocab)
