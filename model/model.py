...
class Model(object):
    ...
    def _build_graph(self):
        ...
        self.logger.info('Time to build graph: {} s'.format(
    ...
    def _build_setup_placeholders(self):
        self.document1 = tf.placeholder(
            tf.int32, [None, self.args.max_document_len])
        self.document2 = tf.placeholder(
            tf.int32, [None, self.args.max_document_len]
        )
        ...
        with tf.variable_scope('embedding'):
            with tf.device('cpu:0'):
                self.word_embeddings = tf.Variable(
                    tf.random_uniform([self.vocab.size(), self.args.embedding_size], -1.0, 1.0),
                    name='word_embeddings',
                    trainable=True
                )

                self.character_embeddings = tf.Variable(
                    tf.random_uniform([self.character_vocab.size(), self.args.character_embedding_size]),
                ...
                '''
            self.document1_emb = tf.nn.embedding_lookup(self.word_embeddings, self.document1)
            self.document2_emb = tf.nn.embedding_lookup(self.word_embeddings, self.document2)
            if self.args.char:
                ...
                self.document1_character_emb = tf.reshape(self.document1_character_emb,
                                                  [-1,self.args.max_word_len,self.args.character_embedding_size])
                self.document2_character_emb = tf.reshape(self.document2_character_emb,
                                                  [-1, self.args.max_word_len, self.args.character_embedding_size])

                document1_character_conv = conv(self.document1_character_emb, self.args.hidden_size,
                          bias=True, activation=tf.nn.relu, kernel_size=5, name="char_conv", reuse=None)
                document1_character_conv = tf.reduce_max(document1_character_conv, axis=1)
                document1_character_conv = tf.reshape(document1_character_conv, [-1, self.args.max_document_len, self.args.hidden_size])

                document2_character_conv = conv(self.document2_character_emb, self.args.hidden_size,
                                  bias=True, activation=tf.nn.relu, kernel_size=5, name="char_conv", reuse=True)
                document2_character_conv = tf.reduce_max(document2_character_conv, axis=1)
                document2_character_conv = tf.reshape(document2_character_conv,
                                                  [-1, self.args.max_document_len, self.args.hidden_size])
                self.doc1 = highway(tf.concat([self.document1_emb, document1_character_conv], axis=2),
                                size=self.args.hidden_size,
                                scope='highway',
                                dropout=self.args.dropout)
                self.doc2 = highway(tf.concat([self.document2_emb, document2_character_conv], axis=2),
                                size=self.args.hidden_size,
                                scope='highway',
                                dropout=self.args.dropout,
                                reuse=True)
    ...
    def _encode(self):
        if self.args.class_model=='rcnn':
            ...
            raise NotImplementedError('Do not implement {} model'.format(self.args.class_model))
        self.document1_represent, self.document2_represent = self.model.build_graph()

    def _match(self):
        with tf.variable_scope('match'):

            self.vector = tf.concat([self.document1_represent,self.document2_represent],1)
            self.score = tc.layers.fully_connected(self.vector,num_outputs=2,activation_fn=tf.nn.tanh)
    ...
    def _train_epoch(self, train_batch):
        ...
        for idx, batch in enumerate(train_batch, 1):
            ...
            _, loss, accuracy, score, vector, predict, doc1, doc2, a,b = self.sess.run(
                    [self.train_op, self.loss, self.accuracy, self.score, self.vector, self.predict, self.document1_represent,
                     self.document2_represent, self.document1_emb, self.document2_emb], feed_dict)
           ...
           # total_auc += auc * len(batch['raw_data'])
            index += 1
            self.logger.info(
                'batch {}, loss {}, accuracy {}'.format(
                    idx, loss, accuracy))

        return 1.0 * total_loss / float(index)
    ...
    def train(
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_batchs = data.get_mini_batchs(batch_size=batch_size, set_name='train')
            train_loss = self._train_epoch(train_batchs)
            self.logger.info(
            ...
            if evaluate:
                if data.dev_set is not None:
                    dev_batches = data.get_mini_batchs(batch_size=batch_size, set_name='dev')
                    loss_, accuracy_,predicts, labels, scores, vectors, document1_represent, document2_represent = self.evaluate(dev_batches)
                    ...
                    self.logger.info('classification_report: \n {}'.format(
                        metrics.classification_report(np.array(labels), np.array(predicts))))
                    self.logger.info("混淆矩阵为: \n {}".format(
                        metrics.confusion_matrix(np.array(labels), np.array(predicts))))
                    self.logger.info('completeness_score: {}'.format(
                        metrics.completeness_score(np.array(labels), np.array(predicts))))
    ...
    def evaluate(
        ...
        scores = []
        vectors = []
        document1_represents = []
        document2_represents = []
        for idx, batch in enumerate(batch_data):
            ...
            loss, accuracy, predict, score, vector, document1_represent, document2_represent = self.sess.run(
                [self.loss, self.accuracy, self.predict, self.score, self.vector, self.document1_represent, self.document2_represent], feed_dict)
            index += 1
            ...
            document1_represents.append(document1_represent.tolist())
            document2_represents.append(document2_represent.tolist())
            '''
            total_auc += auc * len(batch['raw_data'])
            total_accuracy += accuracy * len(batch['raw_data'])
            '''
            ...
            if save_predict_label:
                for sample in batch['raw_data']:
                    result.append(
                        {'document1': sample['document1'],
                         'document2': sample['document2'],
                         'label': sample['label']
                         }
                    )
        ...
        if save_predict_label:
            if result_dir is not None and result_prefix is not None:
                ...
                self.logger.info(
                    "Write predict label to {}".format(result_file))
                with open(result_file, 'w') as fout:
                    for tmp in result:
                        fout.write(json.dumps(tmp, ensure_ascii=False) + "\n")

        return total_loss / float(total_num), total_accuracy / float(index), predicts, labels, scores, vectors, document1_represents, document2_represents
    ...
    def save(self, model_dir, model_prefix):
        ...
        self.logger.info(
            'Model saved in {}, with prefix {}.'.format(
    ...
    def restore(self, model_dir, model_prefix):
        ...
        self.logger.info(
            'Model restored from {}, with prefix {}'.format(
                model_dir, model_prefix))