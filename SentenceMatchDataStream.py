...
def read_all_instances(inpath, word_vocab=None, label_vocab=None, char_vocab=None, max_sent_length=100,
    ...
    for line in infile:
        ...
        if max(word_idx_1) > max_id:
            max_id = max(word_idx_1)
        word_idx_2 = word_vocab.to_index_sequence(sentence2)
        if max(word_idx_2) > max_id:
            max_id = max(word_idx_2)
    ...
    infile.close()
    print('max word id {}'.format(str(max_id)))
    return instances

def read_all_test_instances(inpath, word_vocab=None, char_vocab=None, max_sent_length=100,
                       max_char_per_word=10, isLower=True):
    instances = []
    infile = open(inpath, 'rt')
    idx = -1
    ...
    for line in infile:
        ...
        if len(word_idx_2) > max_sent_length:
            word_idx_2 = word_idx_2[:max_sent_length]
            if char_vocab is not None: char_matrix_idx_2 = char_matrix_idx_2[:max_sent_length]
        instances.append((sentence1, sentence2, word_idx_1, word_idx_2, char_matrix_idx_1, char_matrix_idx_2, cur_ID))
    infile.close()
    print('max word id {}'.format(str(max_id)))
    return instances
...
class SentenceMatchDataStream(object):
    def __init__(self, inpath, word_vocab=None, char_vocab=None, label_vocab=None,
                 isShuffle=False, isLoop=False, isSort=True, options=None, test=False):
        if test:
            instances = read_all_test_instances(
                inpath,
                word_vocab=word_vocab,
                char_vocab=char_vocab,
                max_sent_length=options.max_sent_length,
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batch_spans):
            ...
            if test:
                cur_batch = TestInstanceBatch(cur_instances, with_char=options.with_char)
            else:
...
class TestInstanceBatch(object):
    def __init__(self, instances, with_char=False):
        self.instances = instances
        self.batch_size = len(instances)
        self.question_len = 0
        ...
        for (sentence1, sentence2, word_idx_1, word_idx_2, char_matrix_idx_1, char_matrix_idx_2, cur_ID) in instances:
            cur_question_length = len(word_idx_1)
            cur_passage_length = len(word_idx_2)
            if self.question_len < cur_question_length: self.question_len = cur_question_length
            if self.passage_len < cur_passage_length: self.passage_len = cur_passage_length
